...
with open(os.path.join(this_dir, "locations_generated2.py"), "w+") as regPyFile:
            ...
            for index in range(0,max_bingo):
...
with open(os.path.join(os.path.dirname(this_dir),"pack","jsons","locations.json"),"w+") as pack_locations:
    pack_locations.write("[{\n")
    pack_locations.write('"name":"Bingo Board",\n')
    pack_locations.write('"children":[\n')

    for i in range(max_bingo):
        pack_locations.write(f'    {{"name":"Bingo: Row {i+1}","sections":[{{"name":"Bingo: Row {i+1}"}}],"map_locations":[{{"map":"overview","x":50,"y":{100+50*i}}}]}},\n')
        pack_locations.write(f'    {{"name":"Bingo: Column {i+1}","sections":[{{"name":"Bingo: Column {i+1}"}}],"map_locations":[{{"map":"overview","x":{100+50*i},"y":50}}]}},\n')
        for j in range(max_bingo):
            pack_locations.write(f'    {{"name":"Bingo: R{i+1}C{j+1}","sections":[{{"name":"Bingo: R{i+1}C{j+1}"}}],"map_locations":[{{"map":"overview","x":{100+50*j},"y":{100+50*i}}}]}},\n')

    pack_locations.write(f'    {{"name":"Bingo: Forward Diagonal","sections":[{{"name":"Bingo: Forward Diagonal"}}],"map_locations":[{{"map":"overview","x":50,"y":{100+50*max_bingo}}}]}},\n')
    pack_locations.write(f'    {{"name":"Bingo: Reverse Diagonal","sections":[{{"name":"Bingo: Reverse Diagonal"}}],"map_locations":[{{"map":"overview","x":50,"y":50}}]}}\n')
