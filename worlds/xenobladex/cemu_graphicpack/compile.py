...
# Set mod name
try:
    with open(f"rules.txt", "r") as f:
        rules = f.read()
except: raise Exception("No rules.txt file")
match = re.search(r"^\s*[nN]ame\s*=\s*[\'\"]?(.*?)[\'\"]?\s*$", rules, re.M)
if match is not None: modname = match.group(1) 
else: raise Exception("Rules.txt file contains no name")
...
for filename in (os.path.splitext(file)[0] for file in os.listdir() if file.endswith(".cpp")):
    ...
    try: 
        with open(f"{filename}.cpp", "r") as f:
            data = f.read()
    except: raise Exception(f"Unable to read from {filename}.cpp")
    ...
    except: raise Exception(f"Assembly for {filename}.cpp failed with {res.status_code}")
    res = res.json()
    ...
    # Prepare packages
    packages = [match.groupdict() for match in re.compile(r"^#ifdef (?P<name>.*?)\nmoduleMatches *= *(?P<ids>.*?) *,? *[#;] *(?P<versions>.*?)\n(?P<content>.*?)^#endif *\n", re.MULTILINE | re.DOTALL).finditer(data)]
    ...
    for package in packages:
        content += f'[{modname}_{filename}_{package["name"]}]\nmoduleMatches = {package["ids"]} # {package["versions"]}\n{package["content"]}\n\n'

    # Create file
    try:
        with open(f"patch_{filename}.asm", "w") as f:
            f = f.write(content)
    except: raise Exception(f"Unable to write to patch_{filename}.asm")
