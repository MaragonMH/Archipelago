...
#include <cstddef>
char _formatEnemyText[] = "EN Id=%03x Dc=%01x:";  // First %05x Second %05x Third %05x Fourth %05x Fifth %05x Sixth %05x Seventh %05x:";
...
int getEnBookDiscovery(int id); // ::Util
int getEnBookDefeat(int id); // ::Util
// End

void _postCurl(char[]);

char* _postEnemyList(char* stringStartPtr, char* stringCurrentPtr, char* stringEndPtr, int maxEntrySize) {
    for(int enemyId = 1; enemyId < 1404; enemyId++){
